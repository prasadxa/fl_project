...
def get_current_admin(credentials: HTTPBasicCredentials = Depends(security)):
    """Authenticate admin access using HTTP Basic Auth."""
    ...
    if not admin_user or not admin_pass:
        raise HTTPException(
            status_code=500,
            detail="Server configuration error: Admin credentials are not set in the environment.",
        )
...
@app.get("/api/admin/stats", tags=["Admin"], dependencies=[Depends(get_current_admin)])
def admin_stats():
    """Return aggregate statistics from the SQLite database."""
    return get_db().stats()


@app.get("/api/admin/feedback", tags=["Admin"], dependencies=[Depends(get_current_admin)])
def admin_feedback(
    limit: int = 50,
    offset: int = 0,
...
@app.get("/api/admin/export-csv", tags=["Admin"], dependencies=[Depends(get_current_admin)])
...
@app.get("/api/admin/export-excel", tags=["Admin"], dependencies=[Depends(get_current_admin)])
...
@app.get("/api/admin/sessions", tags=["Admin"], dependencies=[Depends(get_current_admin)])
