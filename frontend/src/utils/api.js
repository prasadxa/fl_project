...
async function request(path, opts = {}) {
  ...
  if (res.status === 401 && path.startsWith("/admin")) {
    const credentials = window.prompt("Admin credentials required (format: username:password)");
    if (credentials) {
      const authStr = btoa(credentials);
      localStorage.setItem("admin_auth", authStr);
      opts.headers = { ...opts.headers, Authorization: `Basic ${authStr}` };
