...
## 2026-06-04 - Skip Links with Sticky Headers
**Learning:** When styling 'Skip to main content' links over sticky headers, use `focus:z-[100]` to ensure visibility. Additionally, the target `<main>` container needs `tabIndex={-1}` to receive focus and `outline-none` to prevent unwanted focus rings.
**Action:** Always include high z-index and proper focus management on target containers when implementing skip links.
