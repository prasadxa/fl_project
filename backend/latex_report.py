...
def tex_escape(text: str) -> str:
    """Escape LaTeX special characters to prevent LaTeX injection."""
    text = str(text)
    # Using a single-pass regex to avoid double-escaping
    conv = {
        '&': r'\&',
        '%': r'\%',
        '$': r'\$',
    ...
    regex = re.compile('|'.join(re.escape(k) for k in sorted(conv.keys(), key=lambda x: -len(x))))
...
\\begin{{center}}
    ...
    \\textbf{{Date:}} {tex_escape(req.server_timestamp)} \\quad \\textbf{{Session:}} {tex_escape(req.session_id[:16])}
\\end{{center}}
\\hrule \\vspace{{0.5cm}}
\\textbf{{Patient Name:}} {tex_escape(req.patient.patient_name)} \\\\
\\textbf{{Patient ID:}} {tex_escape(req.patient.patient_id)} \\\\
\\textbf{{DOB:}} {tex_escape(req.patient.date_of_birth)} \\quad \\textbf{{Gender:}} {tex_escape(req.patient.gender)}
