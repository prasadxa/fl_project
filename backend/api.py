...
def verify_admin(credentials: HTTPBasicCredentials = Depends(security)) -> str:
    """Verify HTTP Basic credentials against ADMIN_USER and ADMIN_PASS environment variables."""

    admin_user = os.getenv("ADMIN_USER")
    admin_pass = os.getenv("ADMIN_PASS")

    # Fail secure if env vars are missing
    if not admin_user or not admin_pass:
        raise HTTPException(status_code=500, detail="Admin credentials not configured")

    correct_user = secrets.compare_digest(credentials.username.encode("utf8"), admin_user.encode("utf8"))
    correct_pass = secrets.compare_digest(credentials.password.encode("utf8"), admin_pass.encode("utf8"))
