...
def escape_latex(s: str) -> str:
    """Escapes special characters for LaTeX using a translation table to avoid double-escaping bugs."""
    if s is None:
        return ""
    s = str(s)
    latex_special_chars = {
...
def build_latex_report(req: ReportRequest) -> bytes:
    with tempfile.TemporaryDirectory() as d:
        ...
        safe_sn = escape_latex(sn)
        safe_risk = escape_latex(risk)
        safe_timestamp = escape_latex(req.server_timestamp)
...
\\begin{{center}}
    ...
    \\textbf{{Date:}} {safe_timestamp} \\quad \\textbf{{Session:}} {safe_session_id}
\\end{{center}}
\\hrule \\vspace{{0.5cm}}
