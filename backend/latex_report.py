...
def _escape_latex(text: str) -> str:
    ...
    regex = re.compile('|'.join(re.escape(key) for key in sorted(conv.keys(), key=lambda x: -len(x))))
...
def build_latex_report(req: ReportRequest) -> bytes:
    with tempfile.TemporaryDirectory() as d:
        ...
        sn = _escape_latex(SHORT_NAMES.get(req.ai_pred_key, req.ai_pred_key))
        risk = _escape_latex(RISK_LEVEL.get(req.ai_pred_key, "UNKNOWN"))
        
        probs = "\n".join([f"\\item \\textbf{{{_escape_latex(SHORT_NAMES.get(k, k))}}}: {v*100:.1f}\\%" for k, v in req.probabilities.items()])
        
        server_timestamp = _escape_latex(req.server_timestamp)
        session_id = _escape_latex(req.session_id[:16])
        patient_name = _escape_latex(req.patient.patient_name)
        patient_id = _escape_latex(req.patient.patient_id)
        date_of_birth = _escape_latex(req.patient.date_of_birth)
        gender = _escape_latex(req.patient.gender)
