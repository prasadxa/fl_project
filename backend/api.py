...
def verify_admin_credentials(credentials: HTTPBasicCredentials = Depends(security)):
    ...
    if correct_username is None or correct_password is None:
        raise HTTPException(
            status_code=500,
            detail="Admin credentials not configured on the server. Please set ADMIN_USER and ADMIN_PASS.",
        )
...
def admin_stats(username: str = Depends(verify_admin_credentials)):
...
def admin_export_csv(username: str = Depends(verify_admin_credentials)):
...
def admin_export_excel(username: str = Depends(verify_admin_credentials)):
    """
...
def admin_sessions(username: str = Depends(verify_admin_credentials), limit: int = 50, offset: int = 0):
