...
export async function downloadCSV() {
  ...
  let res = await fetch(`${BASE}/admin/export-csv`, opts);
  if (res.status === 401) {
    const user = window.prompt("Admin Username:");
    const pass = window.prompt("Admin Password:");
    if (user && pass) {
      const newAuth = btoa(`${user}:${pass}`);
      localStorage.setItem("admin_auth", newAuth);
      opts.headers["Authorization"] = `Basic ${newAuth}`;
      res = await fetch(`${BASE}/admin/export-csv`, opts);
    }
  }
...
export async function downloadExcel() {
  ...
  const auth = localStorage.getItem("admin_auth");
  if (auth) opts.headers["Authorization"] = `Basic ${auth}`;
  let res = await fetch(`${BASE}/admin/export-excel`, opts);
  if (res.status === 401) {
    const user = window.prompt("Admin Username:");
    const pass = window.prompt("Admin Password:");
    if (user && pass) {
      ...
      localStorage.setItem("admin_auth", newAuth);
      opts.headers["Authorization"] = `Basic ${newAuth}`;
      res = await fetch(`${BASE}/admin/export-excel`, opts);
    }
  }
  if (!res.ok) throw new Error("Export failed");
  const blob = await res.blob();
