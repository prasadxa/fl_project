...
def _latex_escape(text: str) -> str:
    """Escapes text for use in a LaTeX document to prevent injection."""
    text = str(text)
    # Define escaping rules for special LaTeX characters
    conv = {
        '&': r'\&',
        '%': r'\%',
        '$': r'\$',
    ...
    # Create a regex to match all characters that need to be escaped
    regex = re.compile('|'.join(re.escape(str(key)) for key in conv.keys()))
    # Replace the matches with their corresponding escaped version
...
def build_latex_report(req: ReportRequest) -> bytes:
    with tempfile.TemporaryDirectory() as d:
        ...
        sn = _latex_escape(SHORT_NAMES.get(req.ai_pred_key, req.ai_pred_key))
        risk = _latex_escape(RISK_LEVEL.get(req.ai_pred_key, "UNKNOWN"))
        
        probs = "\n".join([f"\\item \\textbf{{{_latex_escape(SHORT_NAMES.get(k, k))}}}: {v*100:.1f}\\%" for k, v in req.probabilities.items()])
        
        server_timestamp = _latex_escape(req.server_timestamp)
        session_id = _latex_escape(req.session_id[:16])
        patient_name = _latex_escape(req.patient.patient_name)
        patient_id = _latex_escape(req.patient.patient_id)
        date_of_birth = _latex_escape(req.patient.date_of_birth)
        gender = _latex_escape(req.patient.gender)
...
\\begin{{center}}
    ...
    \\textbf{{Date:}} {server_timestamp} \\quad \\textbf{{Session:}} {session_id}
\\end{{center}}
\\hrule \\vspace{{0.5cm}}
\\textbf{{Patient Name:}} {patient_name} \\\\
\\textbf{{Patient ID:}} {patient_id} \\\\
\\textbf{{DOB:}} {date_of_birth} \\quad \\textbf{{Gender:}} {gender}
