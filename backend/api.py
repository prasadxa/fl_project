...
def get_admin(credentials: HTTPBasicCredentials = Depends(security)):
    # SECURITY: Do not hardcode secrets, fail securely if env vars are missing
...
@app.get("/api/admin/stats", tags=["Admin"], dependencies=[Depends(get_admin)])
def admin_stats():
    """Return aggregate statistics from the SQLite database."""
    return get_db().stats()


@app.get("/api/admin/feedback", tags=["Admin"], dependencies=[Depends(get_admin)])
def admin_feedback(
    limit: int = 50,
    offset: int = 0,
...
@app.get("/api/admin/export-csv", tags=["Admin"], dependencies=[Depends(get_admin)])
...
@app.get("/api/admin/export-excel", tags=["Admin"], dependencies=[Depends(get_admin)])
...
@app.get("/api/admin/sessions", tags=["Admin"], dependencies=[Depends(get_admin)])
